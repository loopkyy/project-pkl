...
$kelurahan = isset($_GET['kode']) ? mysqli_real_escape_string($koneksi, $_GET['kode']) : '';

if (empty($kelurahan)) {
    die("Kelurahan tidak ditemukan.");
}

// Query untuk mendapatkan data penduduk di kelurahan tersebut
$sql = "SELECT nik, nama, kecamatan, kelurahan FROM tb_pdd WHERE kelurahan = '$kelurahan'";
...
$pdf->SetSubject('Data Penduduk Kelurahan');
$pdf->SetKeywords('TCPDF, PDF, example, test, guide');
$pdf->AddPage();

// Judul
$pdf->SetFont('helvetica', 'B', 16);
$pdf->Cell(0, 10, 'Data Penduduk Kelurahan ' . htmlspecialchars($kelurahan), 0, 1, 'C');
...
$pdf->Cell(40, 10, 'Kelurahan', 1); // Lebar kolom Kelurahan
$pdf->Ln();
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell(40, 10, htmlspecialchars($row['kelurahan']), 1);
        $pdf->Ln();
    }
} else {
    // Jika tidak ada data
    $pdf->Cell(0, 10, 'Tidak ada data untuk kelurahan ini.', 1, 1, 'C');
}
...
$pdf->Output('Data_Penduduk_' . htmlspecialchars($kelurahan) . '.pdf', 'I');
